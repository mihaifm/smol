...
function init() {
  ...
  if (!fs.existsSync(`${builder.dirs.db}/feedconfig.json`))
    fs.writeFileSync(`${builder.dirs.db}/feedconfig.json`, JSON.stringify(defaultFeed));
}

function build() {
  var builder = require('./builder');
  var feedConfigFile = `${builder.dirs.db}/feedconfig.json`;
